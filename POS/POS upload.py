...
def taxSummary(docno):
    summary = {}
    sql = ''' select h.netamount, h.taxamount, h.grossamount, COUNT(b.invnumber) from _cplURApos h inner 
    join _cplUraPOSLines b on h.docno = b.invnumber where h.docno = ? AND b.itemprice > 0
...
def sendInvoice():
    ...
    cursor.execute(
        "Select top 50 * From _cplurapos Where sentstatus ='PENDING' and docname = 'inv'")
    ...
    for invoice in docnoList:
        ...
        if invoiceres['returnStateInfo']['returnMessage'] == 'SUCCESS':
            ...
            sql3 = '''update invnum set ucIDPOSInvURAinvoiceID = ?, ucIDPOSInvURAInvoiceNo = ?, ucIDPOSInvURAstatus=?, ucIDPOSInvverificationcode=? where invnumber = ?'''
            values3 = (invoiceID, invoiceNo, responseStatus, antifakeCode,
                       invoice[18])
            cursor.execute(sql3, values3)
            conn.commit()

            sql1 = '''UPDATE _cplURApos SET sentstatus = 'SUCCESS', sentlastupdate= ?, qrcode = ? WHERE docno = ? '''
            values1 = (dt, qrCode, invoice[18])
            cursor.execute(sql1, values1)

            conn.commit()
            sql = '''update invnum set ucIDPOSInvQRcode = ? where invnumber = ? '''
            ...
            cursor.execute(sql3, values3)
            conn.commit()
            sql1 = '''UPDATE _cplURApos SET sentstatus = 'PENDING', sentlastupdate= ? WHERE docno = ? '''
            values1 = (dt, invoice[18])
            cursor.execute(sql1, values1)
            conn.commit()
            logSummary(
