...
def getGoods(docno):
    ...
    sql = '''SELECT [orderlineid],[orderid],[itemname],[itemcode],[itemqty],[itemuom],[itemprice],[linetotprice]
             ...
             ,[taxamount],ROW_NUMBER as row,round(discount,2), discounttaxrate
    ...
    for items in cursor:
        invoiceitems = {
            ...
            "taxRate": f"{items[8]}",
            "tax": f"{items[19]}",
            "discountTotal": f"{str(items[21]).replace('None','')}",
            "discountTaxRate": f"{items[22]}",
