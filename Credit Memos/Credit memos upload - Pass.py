...
def sendInvoice():
    docnoList = []
    # executes the stored procedure for the [_cplURACreditMemo] and _cplUraInvLines tables
    cursor.execute('EXEC CreditMemosURA')
    ...
    cursor.execute(
        "Select top 50 * From _cplURACreditMemo Where sentstatus ='PENDING' and docname = 'inv'")
    ...
    for invoice in docnoList:
        ...
        if invoiceres['returnStateInfo']['returnMessage'] == 'SUCCESS':
            ...
            sql3 = '''update invnum set ucIDCrnCreditNoteID = ?, ucIDCrnCreditNoteInvoiceNo = ?, ucIDCrnURAStatus=?, ucIDCrnverificationCode=? where invnumber = ?'''
            values3 = (invoiceID, invoiceNo, responseStatus, antifakeCode,
                       invoice[18])
            cursor.execute(sql3, values3)
            conn.commit()

            sql1 = '''UPDATE [_cplURACreditMemo] SET sentstatus = 'SUCCESS', sentlastupdate= ?, qrcode = ? WHERE docno = ? '''
            values1 = (dt, qrCode, invoice[18])
            cursor.execute(sql1, values1)

            conn.commit()
            sql = '''update invnum set ucIDCrnqrcode = ? where invnumber = ? '''
            ...
            cursor.execute(sql3, values3)
            conn.commit()
            sql1 = '''UPDATE [_cplURACreditMemo] SET sentstatus = 'PENDING', sentlastupdate= ? WHERE docno = ? '''
            values1 = (dt, invoice[18])
            cursor.execute(sql1, values1)
            conn.commit()
            logSummary(
