...
def sendStockUpdate():
    ...
    sql = '''select * from _cplurastocks where URAstatus = ? and operationType = ? '''
    values = ('PENDING','102')
    ...
    for row in grvList:
        finalJson = {
            "goodsStockIn": {
                ...
                "supplierName": "",
                "adjustType": f"{row[8]}",
                "remarks": f"decrease inventory {row[6]}",
                "stockInDate": "",
        ...
        for grvResponse in responses:
            if grvResponse['returnStateInfo']['returnMessage'] == 'SUCCESS':
                ...
                if content_decoded == []:
                    ...
                    cursor.execute(sql, values)
                    conn.commit()
                    logSummary(f"{row[6]} has been synced with Efris - out")
                    ...
                    values = (str(content_decoded[0]['returnMessage']),dt, row[6])
                    cursor.execute(sql, values)
                    conn.commit()
                    logSummary(f"{row[6]} has failed to synced with Efris")
                    print(f"{row[6]} has failed to synced with Efris due to {(content_decoded[0]['returnMessage'])} ")
                    logSummary("\n \n \n")
            else:
                ...
                cursor.execute(sql, values)
                conn.commit()
                logSummary(f"{row[6]} has failed to synced with Efris - out")
                print(f"{row[6]} has failed to synced with Efris due to {response1} ")
                logSummary("\n \n \n")
